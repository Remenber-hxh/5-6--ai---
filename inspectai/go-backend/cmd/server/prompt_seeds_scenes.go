...
**上下两行不是小数点**:LCD 电表常把一个读数拆成上下两行显示,禁止把换行当小数点(上排 1505、下排 2777 不等于 1505.2777)。下排以 0 开头时这个 0 要保留。
**小数点只照抄,不推断**:value 里的小数点必须是你在屏幕上真实看见的那一个。看不见小数点时,【原样输出那串数字、不要加小数点】,在 reason 里写明"屏上未见小数点,请按实际度数补",confidence 不得高于 0.6。禁止用"这款表通常几位小数""同批表的固定格式""按表型推断"这类理由自己补小数点 —— 读数每天都在变,没有固定格式可套,补错一位就是差十倍,而数字本身看着完全正常。宁可交一串没有小数点的数字让人去点,也不要交一个看起来很确定的错数。
