...
func (s *SQLiteStore) ListReportTemplates() ([]ReportTemplate, error) {
	rows, err := s.db.Query(`SELECT id, name, project, asset_type, max_images,
		...
		prompt_mode, raw_text, scene_features, extra_notes FROM report_templates
	...
	for rows.Next() {
		...
		// 而后台改的模板全部不生效。
		var photosJSON, scene, promptMode, rawText, sceneFeatures, extraNotes sql.NullString
		if err := rows.Scan(&t.ID, &t.Name, &t.Project, &t.AssetType, &t.MaxImages,
			&t.MinImages, &featured, &hasAI, &t.AIPrompt, &scene, &photosJSON,
			&promptMode, &rawText, &sceneFeatures, &extraNotes); err != nil {
			return nil, err
		}
		t.ExtraNotes = extraNotes.String
...
func (s *SQLiteStore) UpsertReportTemplate(t ReportTemplate) error {
	...
	stmt := `INSERT INTO report_templates
		(id,tenant_id,name,project,asset_type,max_images,min_images,featured,has_ai,ai_prompt,disabled,sort_no,updated_at,scene,expected_photos,prompt_mode,raw_text,scene_features,extra_notes)
		VALUES(?,?,?,?,?,?,?,?,?,?,0,0,?,?,?,?,?,?,?)
		...
		scene_features=excluded.scene_features, extra_notes=excluded.extra_notes`
	if s.dialect == "mysql" {
		stmt = `INSERT INTO report_templates
			(id,tenant_id,name,project,asset_type,max_images,min_images,featured,has_ai,ai_prompt,disabled,sort_no,updated_at,scene,expected_photos,prompt_mode,raw_text,scene_features,extra_notes)
			VALUES(?,?,?,?,?,?,?,?,?,?,0,0,?,?,?,?,?,?,?)
			...
			scene_features=VALUES(scene_features), extra_notes=VALUES(extra_notes)`
	}
	photosJSON := ""
	...
	if _, err := tx.Exec(stmt, t.ID, defaultTenantID, t.Name, t.Project, t.AssetType,
		...
		t.SceneFeatures, t.ExtraNotes); err != nil {
