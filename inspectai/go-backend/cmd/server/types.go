...
type ReportTemplate struct {
	...
	// ExtraNotes 这个场景额外要交代给 AI 的话,渲染时【追加】进提示词。
	//
	// 【它存在是为了不让人去整段接管】有些话落不进字段表:比如
	// "防夹/开关门是现场测试项,拍到测试动作就判,别一律留空"——
	// 它不属于任何一个字段,是对整份提示词的补充。
	// 没有这个口子的话,想说这句话就只能切到「整段文本」自己写整封信,
	// 而那样字段表就彻底不参与了,等于为了加一句话放弃了全部结构化配置。
	//
	// 【和字段表共存,不互相覆盖】字段表管「每个字段怎么判」,
	// 这里管「这个场景整体要注意什么」。两个写入口写的是两块内容。
