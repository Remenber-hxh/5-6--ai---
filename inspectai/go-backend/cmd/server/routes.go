...
var apiRoutes = []apiRoute{
	...
	{http.MethodGet, "/api/operation-logs", guardSupervisor, (*Server).handleListOperationLogs},

	// —— 企业微信 ——
	{http.MethodPost, "/api/wework/message", guardSupervisor, (*Server).handleSendWeWorkMessage},
	{http.MethodPost, "/api/wework/group-message", guardSupervisor, (*Server).handleSendWeWorkGroupMessage},

	// —— 工程巡检计划/任务 ——
	{http.MethodGet, "/api/engineering/plans", guardNone, (*Server).handleListEngineeringPlans},
	{http.MethodPost, "/api/engineering/plans", guardNone, (*Server).handleCreateEngineeringPlan},
	{http.MethodGet, "/api/engineering/tasks", guardNone, (*Server).handleListEngineeringTasks},
	{http.MethodPost, "/api/engineering/tasks", guardNone, (*Server).handleCreateEngineeringTask},
	...
	{http.MethodPost, "/api/assets", guardSupervisor, (*Server).handleCreateAsset},
	...
	{http.MethodPost, "/api/management-ai/act", guardSupervisor, (*Server).handleManagementAct},

	// —— 提示词模板 ——
	{http.MethodGet, "/api/prompt/templates", guardNone, (*Server).handleListPromptTemplates},
}

// allow — 表层准入检查。不通过时已写好 403 响应。
func (s *Server) allow(w http.ResponseWriter, r *http.Request, g guard) bool {
	switch g {
