...
> 状态:**两层均已实施**。
> 第一层(2026-07-28,`b4dea9c`):适配层 + 三个危险弹窗路径浏览器实测,打包 55.4kB → 16.3kB gzip。
> 第二层(2026-07-30,`0e07623` + `65acc1d`):统一顶栏 NavBar、流程进度条、浅色页品牌色令牌、
> 卡片系统与空态、首页重排、状态标签收敛、台账封面图。
>
> 第二层里有两项**评估后没按原方案做**,理由见对应章节:
> - **列表没换 Arco Cell** —— 分割线/间距/点按反馈手写版已具备,箭头旧版本来就没有;
>   换 Cell 要覆盖它大部分默认样式才能保住观感。改为补回旧版真正丢掉的设备封面图。
> - **标签没用 Arco Tag** —— 它要把 color/bgColor 当 props 传进 JSX,与本项目
>   「颜色只从令牌取」的规矩冲突。改为收敛成一套 `.tag` + 语义档位。
...
### ⚠️ 坑四:按钮文字以 0 号字渲染 —— 框在、能点、不报错,就是看不见

Arco 的 `.arco-button` **根元素是 `font-size: 0`**,真实字号写在内层
`.arco-button-text` 上。本项目为了统一字号,把内层改成了 `font-size: inherit`
(见 global.css 顶部那一段的原因)。**两条加起来**的后果:凡是没有显式写字号的
按钮,文字都以 0 号字渲染。

**它不报错、不塌布局、按钮照样能点**,所以肉眼审代码看不出来,只有真机上
看到一个空框才知道。

**踩过两次**:
- 2026-08-10「没有待处理照片」那屏的「去拍照」是个空框
- 2026-08-20 扫码页三个按钮全是空的蓝条

第一次的修法是**在那一个按钮上补字号**,所以第二次照样踩。现在 global.css 里
给所有尺寸档的 `.arco-button` 加了**兜底 15px**,业务样式(`.btn-primary` 等)
写在更后面,权重相同、后来居上,照旧覆盖。

> **新写按钮不用再操心字号。** 但如果哪天要动 global.css 顶部那段
> `font-size: inherit`,必须连着看这条兜底 —— 两条是一对的。

### ⚠️ 坑五:把深色皮的样式搬到浅色页 —— 字会看不见

这个 app 有**两套皮**:登录页、拍照页、扫码取景是深色;台账、记录、表单、
设备详情是浅色。global.css 里有一大段是给深色皮写的(白色描边、
`color: var(--ink)`、半透明白的标签、写死 `#fff` 的输入文字)。

**原样复用到浅色页上,就是浅色字配浅色底——看不见。**

**踩过三次**:表单(`.form-light` 就是为此单独拆出来的一整套)、卡片、备案号
(`.flow-screen .beian-line` 那条覆盖)。2026-08-20 扫码页的次要按钮是第四次,
这次在写的时候就避开了。

**规矩:浅色页不要复用深色皮的类,单独压一层。** 判断方法很简单——
看这一页的容器是不是 `.flow-screen`(它是 `--lt-bg` / `--lt-fg`,浅色)。
...
> **先看两条静默坑**:坑四(按钮 0 号字)和坑五(深色皮样式用到浅色页)。
> 这两类的共同点是**不报错、不塌布局、功能照常**,只有真机上看一眼才知道。
> 加了新按钮或新页面之后,一定在手机上把那一屏打开看一次,别只看类型检查。
