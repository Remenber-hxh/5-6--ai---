 ...
 * 页面上没有警示本身就是"正常"。
 */
export default function AssetOpenItems({ assetId }: { assetId: string }) {
  ...
  if (abnormalWithoutTask) {
    return (
      <Alert
        type="error"
        showIcon
        message={`最近一次巡检判为「${data.lastStatus || "异常"}」,但没有任何在办任务`}
        description="也就是说问题被发现了,却没人接手。请在计划页派发一条复查任务,或到台账把它标记为正常。"
      />
    );
  }
