...
4. **confidence 评分标准**：
   ...
   - `0.5-0.7`：勉强看出但需人工复核
   - `<0.5`：不要给出，留空
5. **`kind=number` 的字段必须带上 `imageIndex` 和 `bbox`**（其它 kind 可给可不给）：
   - `imageIndex`：这个值是从第几张图读到的，**从 1 开始**，按 USER 里的上传顺序
   - `bbox`：读数在那张图里的位置，归一化到 0~1 的 `[左, 上, 右, 下]`
   - 框住**读数区本身**（LCD 显示区 / 水表字轮窗口）即可，不要框整台设备、不要框整张图
   - 系统会按这个框把读数区裁出来放大、再核一遍。**框给准了，你这次读错也能被纠回来**；
     框歪了或不给，就只能按你这一遍的结果算。
