...
必须从下表选择最匹配的一项；如果特征不足，返回 `unknown` 并让用户手动选择。

> **这张表在运行时会被后端下发的候选表整段替换**（候选来自数据库里的巡检模板，
> 后台新建的模板会自动出现在里面）。下面这份是**兜底**：只有在后端没下发候选时才用，
> 用于保证下发链路出问题时仍能识别既有场景，而不是一个都认不出来。
...
> 下面提到的 templateId 是历史上就存在的那几个。**以上面的候选表为准** ——
> 表里没有的 id 一律不要返回（后端会把候选外的 id 当成"没认出来"）。
> 这些线索的价值在于"怎么把长得像的两类分开"，那部分不随模板增减而失效。
